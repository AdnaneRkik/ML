...
from numpy import unique as itemfreq
...
def RunClassifiers(data_train, data_test, labels_train, labels_test, experiment, dataset, algos='all', tune='n'):
    ...
    for c, func, tune_parms in classifiers:
        ...
        P_train, R_train, F1_train_w, sup_train = precision_recall_fscore_support(labels_train, pred_train,
                                                                                  pos_label=None, average='weighted',
                                                                                  warn_for=('recall'))
        P_test, R_test, F1_test_w, sup_test = precision_recall_fscore_support(labels_test, pred_test, pos_label=None,
        ...
        results.iloc[res] = [experiment, dataset, c, 'train', acc_train, P_train, R_train, F1_train_w, F1_train_mac,
                             acc_train_0, acc_train_p1, acc_train_m1]  # , F1_train_mic]
        results.iloc[res + 1] = [experiment, dataset, c, 'test', acc_test, P_test, R_test, F1_test_w, F1_test_mac,
                                 acc_test_0, acc_test_p1, acc_test_m1]  # , F1_test_mic]
        res = res + 2
