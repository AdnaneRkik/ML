...
def quote_data_labels(data):
    ...
    zeros = 1 - negs - ones
    print(
        "Labels Generated 0:1:-1 = {}:{}:{}".format(round(zeros * 100, 2), round(ones * 100, 2), round(negs * 100, 2)))
...
def Order_Arrival_Rates(X, y, t_thresh=5):
    ...
    for i, row in enumerate(XT):
        ...
        if not win[:, 0].size == 0:
            ...
            canc_vol_buy_lob = np.sum(
                win[((win[:, 1] == 2) | (win[:, 1] == 3)) & (win[:, 5] == 1) & (win[:, 4] >= win[:, 8])][:, 3])
            canc_vol_sell_lob = np.sum(
                win[((win[:, 1] == 2) | (win[:, 1] == 3)) & (win[:, 5] == -1) & (win[:, 4] <= win[:, 6])][:, 3])
