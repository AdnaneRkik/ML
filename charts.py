...
def print_chart(df, tslice, start_time=-1, end_time=-1):
    ...
    for t in range(int(n_ints)):
        ...
        if not df_sl.empty:
            ...
            df_sell_create_sl = df_sell_create[
                (df_sell_create['time'] >= int_start) & (df_sell_create['time'] < int_end)]
            df_sell_cancel_sl = df_sell_cancel[
                (df_sell_cancel['time'] >= int_start) & (df_sell_cancel['time'] < int_end)]
